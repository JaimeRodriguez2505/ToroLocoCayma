  ...
  useEffect(() => {
    ...
    const prepareDocument = async () => {
      ...
      if (pdfBlob) {
        ...
        try {
          // Importar las funciones de conversión dinámicamente
          const { convertHtmlToPdf, generateSimplePdf } = await import("../services/invoiceService")

          try {
            console.log("🔄 Intentando conversión completa a PDF...")
            ...
            console.log("🔄 Intentando PDF simple como fallback...")
            ...
            try {
              ...
              console.error("❌ Error en PDF simple:", fallbackError)
              if (isMounted) {
          ...
          if (isMounted) {
            setErrorMessage("No se pudo cargar el servicio de conversión.")
            setIsPdfGenerating(false)
            setIsLoading(false)
          }
        }
